...
async def go_to_note(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(message_text,
                                     reply_markup=user_keyboards.get_note_operations_kb(note['_id']),
                                     parse_mode="HTML")
...
async def get_edited_title(message: Message, state: FSMContext):
    ...
    database_operations.update_note_title(note_data['note_id'], message.text, message.from_user.username)
